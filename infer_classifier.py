...
parser.add_argument(
    "--input", required=True, help="directory containing images for inference"
)
parser.add_argument(
    "--config", required=True, help="path to the config file containing the class names"
)
parser.add_argument(
    "--model-name",
    dest="model_name",
...
if __name__ == "__main__":
    ...
    infer_result_path = os.path.join("outputs", args.out_dir)
    os.makedirs(infer_result_path, exist_ok=True)

    checkpoint = torch.load(weights_path)
    # Load the model.
    model = Dinov3Classification(
        num_classes=len(CLASS_NAMES), model_name=args.model_name, repo_dir=DINOV3_REPO
    ).to(DEVICE)
    ...
    for i, image_path in enumerate(all_image_paths):
        ...
        result = inference(model, image, DEVICE, orig_image)
