...
run_seg_experiment() {
  ...
  echo "========================================" | tee -a "$out_dir/train.log"
  echo "BATCH_SIZE=$BATCH_SIZE NUM_WORKERS=$NUM_WORKERS IMG_SIZE=${IMG_WIDTH}x${IMG_HEIGHT} MAX_EPOCH=$MAX_EPOCH EARLY_STOPPING_MONITOR=$EARLY_STOPPING_MONITOR PATIENCE=$EARLY_STOPPING_PATIENCE" | tee -a "$out_dir/train.log"
