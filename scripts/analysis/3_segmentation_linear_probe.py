#!/usr/bin/env python3
"""
Analysis 3 — Segmentation Linear Probe

Strict linear probe definition:
- frozen last-layer normalized patch tokens
- 1x1 conv classifier
...
def setup_console_and_file_logging(
    ...
    if log_file_arg:
        ...
        log_path = output_root / f"3_segmentation_linear_probe_{stamp}.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analysis 3 — Segmentation Linear Probe")
    ...
    parser.add_argument("--output-root", default="outputs/8_segmentation_linear_probe")
...
def main() -> None:
    ...
    try:
        ...
        manifests = {
            split_name: build_or_load_manifest(
        ...
        for split_name in ["train", "valid", "test"]:
            targets[split_name] = extract_or_load_targets(
                split_name=split_name,
            ...
            log(f"{split_name}: target_cache_shape={tuple(targets[split_name].shape)}")
        ...
        imagenet_valid_features = extract_or_load_features(
            split_name="valid",
            backbone_name="imagenet",
            ckpt_path=ckpt_imagenet,
        ...
        if resolve_device(args.device).type == "cuda":
            torch.cuda.empty_cache()
        imagenet_test_features = extract_or_load_features(
            split_name="test",
            backbone_name="imagenet",
            ckpt_path=ckpt_imagenet,
        ...
        imagenet_test_metrics, imagenet_test_df = evaluate_probe_split(
            model=imagenet_model,
            backbone_name="imagenet",
            split_name="test",
        ...
        imagenet_examples = collect_example_predictions(
            model=imagenet_model,
        ...
        cag_train_features = extract_or_load_features(
            split_name="train",
            backbone_name="cag",
            ckpt_path=ckpt_cag,
            manifest=manifests["train"],
        ...
        cag_valid_features = extract_or_load_features(
            split_name="valid",
            backbone_name="cag",
            ckpt_path=ckpt_cag,
            manifest=manifests["valid"],
        ...
        if resolve_device(args.device).type == "cuda":
            torch.cuda.empty_cache()
        cag_test_features = extract_or_load_features(
            split_name="test",
            backbone_name="cag",
            ckpt_path=ckpt_cag,
            manifest=manifests["test"],
        ...
        cag_test_metrics, cag_test_df = evaluate_probe_split(
            model=cag_model,
            backbone_name="cag",
            split_name="test",
        ...
        cag_examples = collect_example_predictions(
            model=cag_model,
        ...
        log(f"Saved per-image metrics: {imagenet_per_image_path}")
        log(f"Saved per-image metrics: {cag_per_image_path}")

        lr_search_path = out_root / "lr_search_results.json"
        with open(lr_search_path, "w", encoding="utf-8") as f:
            json.dump(
                {
                    "imagenet": imagenet_lr_search,
                    "cag": cag_lr_search,
                },
                f,
                indent=2,
            )
        log(f"Saved LR search results: {lr_search_path}")
        tracker.finish_step("Save summaries")

        tracker.start_step("Render figures")
        save_bar_compare(summary_df, out_root / "fig_seg_linear_probe_bar_compare.png")
        ...
        run_meta = {
            ...
            "manifests": {
                split_name: {
                    "hash": hash_dataframe(df, ["image_id", "image_path", "mask_path"]),
        ...
        log(f"Saved run meta: {out_root / 'run_meta.json'}")
