#!/usr/bin/env python3
"""
Global Analysis 4-2 — Same-DICOM Retrieval.
...
TARGET_NAME = "same_dicom"
DEFAULT_DATASET_MANIFEST_NAME = "manifest_same_dicom_master.csv"
...
def load_dataset_manifest(image_root: Path, manifest_path: Path) -> pd.DataFrame:
    ...
    manifest["img_path"] = manifest["img_path"].astype(str)
    manifest["split"] = manifest["split"].astype(str)
    manifest["class_name"] = manifest["class_name"].astype(str)
    manifest["patient_id"] = manifest["patient_id"].astype(str)
    manifest["study_id"] = manifest["study_id"].astype(str)
    manifest["dicom_id"] = manifest["dicom_id"].astype(str)
    manifest["dicom_rel_path"] = manifest["dicom_rel_path"].astype(str)
    manifest["source_contrast_png"] = manifest["source_contrast_png"].astype(str)
    manifest["contrast_frame_index"] = pd.to_numeric(manifest["contrast_frame_index"], errors="raise").astype(np.int64)
    manifest["frame_index"] = pd.to_numeric(manifest["frame_index"], errors="raise").astype(np.int64)
    manifest["frame_offset"] = pd.to_numeric(manifest["frame_offset"], errors="raise").astype(np.int64)
    manifest["img_path"] = manifest["img_path"].map(lambda p: str(Path(p).resolve()))
    manifest["source_contrast_png"] = manifest["source_contrast_png"].map(lambda p: str(Path(p).resolve()))

    missing_paths = [path for path in manifest["img_path"].tolist() if not Path(path).exists()]
    if missing_paths:
        raise FileNotFoundError(f"Dataset manifest references missing images: {missing_paths[:10]}")

    actual_image_paths = {str(Path(path).resolve()) for path in collect_image_paths(str(image_root), ("png", "jpg", "jpeg"))}
    manifest_paths = set(manifest["img_path"].tolist())
    if actual_image_paths != manifest_paths:
        missing_from_root = sorted(manifest_paths - actual_image_paths)
        extra_in_root = sorted(actual_image_paths - manifest_paths)
        raise ValueError(
            "Image root and dataset manifest disagree. "
            f"missing_from_root={missing_from_root[:10]} extra_in_root={extra_in_root[:10]}"
        )

    manifest = manifest.sort_values(["split", "class_name", "dicom_id", "frame_offset", "img_path"]).reset_index(drop=True)
    manifest.insert(0, "image_id", np.arange(len(manifest), dtype=np.int64))
    return manifest


def validate_same_dicom_pairs(manifest: pd.DataFrame) -> None:
    if manifest.empty:
        raise ValueError("Dataset manifest is empty.")
    counts = manifest.groupby(["split", "dicom_id"]).size()
    if int(counts.min()) != 2 or int(counts.max()) != 2:
        bad = counts[counts != 2].head(10).to_dict()
        raise ValueError(f"Each split/dicom_id must contain exactly 2 images. Violations: {bad}")
    offsets = manifest.groupby(["split", "dicom_id"])["frame_offset"].apply(lambda s: tuple(sorted(int(v) for v in s.tolist())))
    bad_offsets = offsets[offsets != (-3, 3)]
    if not bad_offsets.empty:
        raise ValueError(f"Unexpected frame offsets in manifest: {bad_offsets.head(10).to_dict()}")
...
def split_manifest_map(manifest: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    for split_name in ["train", "valid", "test"]:
        ...
        counts = split_df["dicom_id"].value_counts()
        if int(counts.min()) != 2 or int(counts.max()) != 2:
            bad = counts[counts != 2].head(10).to_dict()
            raise ValueError(f"Each split manifest must contain exactly 2 images per dicom_id. Violations: {bad}")
...
def get_target_values(manifest: pd.DataFrame, target: str) -> np.ndarray:
    if target == TARGET_NAME:
        return manifest["dicom_id"].to_numpy(dtype=str)
...
def compute_retrieval_metrics(
    ...
    dicom_ids = manifest["dicom_id"].to_numpy(dtype=str)
    frame_offsets = manifest["frame_offset"].to_numpy(dtype=np.int64)
    ...
    for i in range(n):
        ...
        if len(positive_indices) > 1:
            raise ValueError(f"Expected exactly 1 positive for {TARGET_NAME}, got {len(positive_indices)} for {image_paths[i]}")
        ...
        per_query_rows.append(
            {
                ...
                "query_dicom_id": dicom_ids[i],
                "query_frame_offset": int(frame_offsets[i]),
                ...
                "top_dicom_ids": " | ".join(dicom_ids[nn_idx]),
                "top_frame_offsets": " | ".join(str(int(frame_offsets[j])) for j in nn_idx),
...
def save_map_compare_figure(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    plt.ylabel("mAP")
    plt.title("Global Analysis 4-2: Same-DICOM Retrieval mAP")
...
def save_recall_compare_figure(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    plt.ylabel("Recall@K")
    plt.title("Global Analysis 4-2: Same-DICOM Retrieval Recall@K")
    plt.grid(axis="y", alpha=0.25)
    plt.legend(ncols=2)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
...
def save_rank_cdf_figure(
    ...
    plt.title("Global Analysis 4-2: Same-DICOM First Positive Rank CDF")
    plt.grid(alpha=0.25)
    plt.legend(ncols=2)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
...
def save_probe_minus_raw_delta_figure(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    plt.title("Global Analysis 4-2: Probe Improvement over Raw Retrieval")
    plt.grid(axis="y", alpha=0.25)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def write_markdown_summary(output_path: Path, summary_df: pd.DataFrame, run_meta: Dict[str, object]) -> None:
    ...
    lines = []
    lines.append("# Global Analysis 4-2: Same-DICOM Retrieval")
    lines.append("")
    lines.append("## Setup")
    lines.append("")
    lines.append(f"- Dataset: `{run_meta['image_root']}`")
    lines.append(f"- Dataset manifest: `{run_meta['dataset_manifest']}`")
    ...
    lines.append("")
    lines.append("- `raw도 높음` -> same-DICOM grouping is already linearly accessible in frozen CLS features.")
    lines.append("- `raw 약함 + probe 회복` -> same-DICOM signal exists but is poorly organized in frozen global geometry.")
    lines.append("- `raw 약함 + probe도 약함` -> even easy same-DICOM grouping is weak, so later device-driven anchoring may be a more fundamental weakness.")
    lines.append("- `queries_with_no_positive == 0` should always hold for this task because each query has exactly one paired frame.")
    lines.append("")
...
def build_steps(args: argparse.Namespace) -> List[StepDef]:
    ...
    for backbone_name in ["imagenet", "cag"]:
        for split_name in ["valid", "test"]:
            steps.append(StepDef(f"Run raw retrieval | backbone={backbone_name} target={TARGET_NAME} split={split_name}", "short"))
    for seed in args.probe_seeds:
        for backbone_name in ["imagenet", "cag"]:
            steps.append(StepDef(f"Run probe retrieval | seed={seed} backbone={backbone_name} target={TARGET_NAME}", "probe"))
    steps.extend(
        [
...
def main() -> None:
    ...
    parser.add_argument("--image-root", default="input/Stent-Contrast-same-dicom-unique-view")
    parser.add_argument("--dataset-manifest", default=None)
    parser.add_argument("--output-root", default="outputs/global_4_2_same_dicom_retrieval_unique_view")
    ...
    log_path, file_handle, original_stdout, original_stderr = setup_console_and_file_logging(
        out_root,
        args.log_file,
        default_prefix="global_4_2_same_dicom_retrieval",
    )
    ...
    try:
        ...
        dataset_manifest_path = Path(args.dataset_manifest).resolve() if args.dataset_manifest else image_root / DEFAULT_DATASET_MANIFEST_NAME
        steps = build_steps(args)
        run_tracker = AnalysisRunTracker(steps)
        step_idx = 0

        run_tracker.start_step(step_idx)
        manifest = load_dataset_manifest(image_root=image_root, manifest_path=dataset_manifest_path)
        validate_same_dicom_pairs(manifest)
        ...
        for backbone_name in ["imagenet", "cag"]:
            for split_name in ["valid", "test"]:
                ...
                labels = get_target_values(split_df, TARGET_NAME)
                ...
                per_query_df.to_csv(
                    out_root / f"per_query_{TARGET_NAME}_raw_{backbone_name}_{split_name}.csv",
                    index=False,
                )
                if split_name == "test":
                    raw_rank_map[backbone_name] = rank_values
                run_tracker.finish_step()
                step_idx += 1
        ...
        for seed in args.probe_seeds:
            for backbone_name in ["imagenet", "cag"]:
                ...
                split_label_values = {
                    split_name: get_target_values(split_manifests[split_name], TARGET_NAME)
                    for split_name in ["train", "valid", "test"]
                }
                ...
                for split_name in ["valid", "test"]:
                    ...
                    per_query_df.to_csv(
                        out_root / f"per_query_{TARGET_NAME}_probe_seed{seed}_{backbone_name}_{split_name}.csv",
                        index=False,
                    )
                    if split_name == "test":
                        probe_rank_map.setdefault(backbone_name, []).append(rank_values)
                run_tracker.finish_step()
                step_idx += 1
        ...
        raw_summary_df.to_csv(out_root / "summary_global_4_2_retrieval_raw.csv", index=False)
        probe_raw_df.to_csv(out_root / "summary_global_4_2_retrieval_probe_raw.csv", index=False)
        summary_df.to_csv(out_root / "summary_global_4_2_retrieval.csv", index=False)
        ...
        run_tracker.start_step(step_idx)
        save_map_compare_figure(summary_df, out_root / "fig_global4_2_map_compare.png")
        save_recall_compare_figure(summary_df, out_root / "fig_global4_2_recall_at_k_compare.png")
        save_rank_cdf_figure(raw_rank_map, probe_rank_map, out_root / "fig_global4_2_rank_cdf_compare.png")
        save_probe_minus_raw_delta_figure(summary_df, out_root / "fig_global4_2_probe_minus_raw_delta.png")
        run_tracker.finish_step()
        step_idx += 1
        ...
        run_meta = {
            ...
            "manifest_hashes": manifest_hashes,
            "feature_hashes": feature_hashes,
            "raw_summary_path": str((out_root / "summary_global_4_2_retrieval_raw.csv").resolve()),
            "probe_raw_summary_path": str((out_root / "summary_global_4_2_retrieval_probe_raw.csv").resolve()),
            "aggregate_summary_path": str((out_root / "summary_global_4_2_retrieval.csv").resolve()),
            **policy_spec.to_meta(),
        ...
        run_tracker.start_step(step_idx)
        write_markdown_summary(out_root / "analysis_global_4_2_same_dicom_retrieval.md", summary_df, run_meta)
        run_tracker.finish_step()
        step_idx += 1
        ...
        log("Global Analysis 4-2 completed successfully.")
    finally:
