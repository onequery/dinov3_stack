...
def setup_logging(output_root: Path, log_prefix: str) -> tuple[Path, object, object, object]:
    ensure_dir(output_root)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    safe_prefix = log_prefix.strip() or "build_global_5_single_device_unique_view_subset"
    log_path = output_root / f"{safe_prefix}_{stamp}.log"
...
def render_markdown_report(
    ...
    lines.append(f"- target_model_name_raw: `{args.target_model_name}`")
    lines.append(f"- target_model_name_normalized: `{args.target_model_name_normalized}`")
    lines.append("")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a single-device subset of Stent-Contrast unique-view for single-device Global Analysis runs.")
    ...
    parser.add_argument("--analysis-title", type=str, default="Global Analysis 5 Single-Device Subset")
    parser.add_argument("--summary-prefix", type=str, default="")
    parser.add_argument("--markdown-name", type=str, default="analysis_single_device_unique_view_subset.md")
    parser.add_argument("--log-prefix", type=str, default="build_global_5_single_device_unique_view_subset")
    parser.add_argument("--log-every", type=int, default=200)
    parser.add_argument("--overwrite", action="store_true")
    args = parser.parse_args()
    args.target_model_name_normalized = normalize_manufacturer_model_name(args.target_model_name)
    return args
...
def main() -> None:
    ...
    try:
            ...
            + json.dumps(
                {
                    ...
                    "target_model_name_normalized": str(args.target_model_name_normalized),
                    "analysis_title": str(args.analysis_title),
                    "summary_prefix": str(args.summary_prefix),
                    "markdown_name": str(args.markdown_name),
                ...
                if parsed is None:
                    ...
                    if not dcm_path.exists():
                        ...
                        try:
                            ...
                            if normalized_name != args.target_model_name_normalized:
        ...
        summary_prefix = str(args.summary_prefix)
        if summary_prefix:
            manifest.to_csv(output_root / prefixed_name(summary_prefix, "selection_audit.csv"), index=False)
            selected_manifest.to_csv(output_root / prefixed_name(summary_prefix, "manifest_selected.csv"), index=False)
            counts_by_split.to_csv(output_root / prefixed_name(summary_prefix, "summary_selected_counts_by_split.csv"), index=False)
            counts_by_split_stent_label.to_csv(
                output_root / prefixed_name(summary_prefix, "summary_selected_counts_by_split_stent_label.csv"),
                index=False,
            )
            status_counts.to_csv(output_root / prefixed_name(summary_prefix, "summary_selection_status.csv"), index=False)
