...
DEFAULT_EXP_ROOT = (
    REPO_ROOT
    / "outputs/fm_improve_exp1-input_policy/input_v3_histogram_standardization/downstream_only"
)
...
VARIANT_DIR = "input_v3"
VARIANT_POLICY = "input_v3_histogram_standardization"
...
def build_analysis_markdown(summary_df: pd.DataFrame, delta_df: pd.DataFrame) -> str:
    lines: list[str] = []
    lines.append("# input_v3: Histogram Standardization")
    lines.append("")
    lines.append("## Summary")
    lines.append("")
    lines.append("- Scope: downstream-only, benchmark-only, frozen backbone.")
    lines.append("- Variant: direct histogram matching to one shared reference CDF built from unique_view/train.")
    lines.append("- Comparison: baseline_input vs input_v3 within the same experiment root.")
    lines.append("")
...
def main() -> None:
    ...
    summary_path = reports_dir / "summary_input_v3_benchmark_metrics.csv"
    delta_path = reports_dir / "summary_input_v3_metric_deltas.csv"
    analysis_path = reports_dir / "analysis_input_v3_histogram_standardization.md"
