...
DEFAULT_EXP_ROOT = (
    REPO_ROOT
    / "outputs/fm_improve_exp1-input_policy/input_v4_border_suppression/downstream_only"
)
...
VARIANT_DIR = "input_v4"
VARIANT_POLICY = "input_v4_border_suppression"
...
def build_analysis_markdown(summary_df: pd.DataFrame, delta_df: pd.DataFrame) -> str:
    lines: list[str] = []
    lines.append("# input_v4: Border Suppression")
    lines.append("")
    lines.append("## Summary")
    lines.append("")
    lines.append("- Scope: downstream-only, benchmark-only, frozen backbone.")
    lines.append("- Variant: targeted device-specific border suppression for normalized ManufacturerModelName `P H I L I P S INTEGRIS H` using a fixed circular ROI mask plus low-frequency background extrapolation.")
    lines.append("- Comparison: baseline_input vs input_v4 within the same experiment root.")
    lines.append("")
...
def main() -> None:
    ...
    summary_path = reports_dir / "summary_input_v4_benchmark_metrics.csv"
    delta_path = reports_dir / "summary_input_v4_metric_deltas.csv"
    analysis_path = reports_dir / "analysis_input_v4_border_suppression.md"
