...
DEFAULT_SUMMARY_CSV = (
    REPO_ROOT
    / "outputs/fm_improve_exp1-input_policy/input_v4_border_suppression/downstream_only/reports/summary_input_v4_benchmark_metrics.csv"
)
...
def plot_task(summary_df: pd.DataFrame, task: str, split: str, output_dir: Path) -> None:
    ...
    rows = []
    for input_policy_dir in ("baseline_input", "input_v4"):
    ...
    out_csv = output_dir / f"summary_input_v4_{task}_combined_{split}.csv"
    ...
    for ax, (metric_col, metric_label) in zip(axes, cfg["metrics"], strict=True):
        ...
        for backbone_name, readout_mode, label in ORDER:
            ...
            var_match = plot_df[
                (plot_df["input_policy_dir"] == "input_v4")
                & (plot_df["backbone_name"] == backbone_name)
                & (plot_df["readout_mode"] == readout_mode)
            ]
        ...
        ax.bar(x + width / 2, variant_vals, width=width, label="input_v4", color=COLOR_MAP["input_v4"])
    ...
    out_fig = output_dir / f"fig_input_v4_{task}_combined_{split}.png"
